...
const registeradmin = async (req, res, next) => {
    ...
    try {
        await adminModel.find({ mobileno:mobileno }).then((result) => {
            if (result.length > 0) {
                res
                    .status(409)
                    .send({ message: "An account with mobile number X already exists..", status: false });
            } else {
...
const otps = async (req, res) => {
    let password = req.body.password;
    try {
        await messageModel.find({ password: password }).then((result) => {
            if (result.length < 0) {
                res
                    .status(409)
...
module.exports = { glandingpage, registeradmin, gadminsignup, adminlogin, geTdashboard, uploadimage, uploadchats, otps };
